...
const IssueDetail = () => {
  ...
  const handleFormalSubmit = async () => {
    try {
      await axios.put(`https://civictrack-backend-rjpa.onrender.com/api/issues/${id}/status?status=FORMALLY_REPORTED`);
  ...
  useEffect(() => {
    axios.get(`https://civictrack-backend-rjpa.onrender.com/api/issues/${id}`)
      .then(res => {
        setIssue(res.data);
        setLoading(false);
      })
